...
good_app=$(make_app good 12.0)
compile_executable "$good_app/Contents/MacOS/barrier" arm64-apple-macos12 \
...
expect_failure missing-executable 'unable to read CFBundleExecutable' \
    "$verifier" --app "$missing_executable_app" --target 12.0 --arch arm64
...
unsafe_plist_target_app=$(make_app unsafe-plist-target 12.0)
...
whitespace_app=$(make_app whitespace 12.0)
...
printf '%s\n' 'int whitespace_fixture(void) { return 0; }' \
    | /usr/bin/xcrun clang -x c - -target arm64-apple-macos12 -dynamiclib \
...
printf '%s\n' \
    ...
    | /usr/bin/xcrun clang -x c - -target arm64-apple-macos12 \
...
"$verifier" --app "$whitespace_app" --target 12.0 --arch arm64 \
...
printf '%s\n' 'int weak_fixture(void) { return 0; }' \
    | /usr/bin/xcrun clang -x c - -target arm64-apple-macos12 -dynamiclib \
        ...
        -o "$weak_fixture"
weak_app=$(make_app weak-missing 12.0)
...
"$verifier" --app "$weak_app" --target 12.0 --arch arm64 \
...
helper_chain_app=$(make_app helper-chain 12.0)
...
expect_failure helper-chain \
    ...
    "$verifier" --app "$helper_chain_app" --target 12.0 --arch arm64

runpath_order_app=$(make_app runpath-order 12.0)
...
"$verifier" --app "$runpath_order_app" --target 12.0 --arch arm64 \
...
non_dylib_target_app=$(make_app non-dylib-target 12.0)
...
compile_executable \
    ...
    arm64-apple-macos12 'int main(void) { return 0; }'
expect_failure non-dylib-target \
    'unresolved bundled dependency: MacOS/barrier' \
    "$verifier" --app "$non_dylib_target_app" --target 12.0 --arch arm64

multiple_executables_app=$(make_app multiple-executables 12.0)
...
"$verifier" --app "$multiple_executables_app" --target 12.0 --arch arm64 \
...
cycle_app=$(make_app dependency-cycle 12.0)
...
printf '%s\n' 'int cycle_a(void); int cycle_b(void) { return cycle_a(); }' \
    | /usr/bin/xcrun clang -x c - -target arm64-apple-macos12 -dynamiclib \
        ...
        -o "$cycle_app/Contents/Frameworks/libCycleB.dylib"
compile_executable "$cycle_app/Contents/MacOS/barrier" arm64-apple-macos12 \
    'int main(void) { return 0; }'
"$verifier" --app "$cycle_app" --target 12.0 --arch arm64 \
    > "$test_root/dependency-cycle.log"

directory_alias_app=$(make_app directory-alias 12.0)
...
"$verifier" --app "$directory_alias_app" --target 12.0 --arch arm64 \
    > "$test_root/directory-alias.log"

reentry_alias_app=$(make_app reentry-alias 12.0)
...
expect_failure reentry-alias \
    ...
    "$verifier" --app "$reentry_alias_app" --target 12.0 --arch arm64
...
canonical_alias_app=$(make_app canonical-alias 12.0)
...
expect_failure canonical-alias \
    ...
    "$verifier" --app "$canonical_alias_app" --target 12.0 --arch arm64
...
system_traversal_app=$(make_app system-traversal 12.0)
printf '%s\n' \
...
expect_failure system-traversal \
    ...
    "$verifier" --app "$system_traversal_app" --target 12.0 --arch arm64
...
line_separator_app=$(make_app line-separator-command 12.0)
printf '%s\n' 'int main(void) { return 0; }' \
    | /usr/bin/xcrun clang -x c - -target arm64-apple-macos12 \
        -Wl,-headerpad,128 \
...
expect_failure line-separator-command \
    ...
    "$verifier" --app "$line_separator_app" --target 12.0 --arch arm64 \
...
invalid_dylinker_app=$(make_app invalid-dylinker 12.0)
...
expect_failure invalid-dylinker \
    ...
    "$verifier" --app "$invalid_dylinker_app" --target 12.0 --arch arm64

dyld_environment_app=$(make_app dyld-environment 12.0)
printf '%s\n' 'int main(void) { return 0; }' \
    | /usr/bin/xcrun clang -x c - -target arm64-apple-macos12 \
        -Wl,-headerpad,128 \
...
expect_failure dyld-environment \
    ...
    "$verifier" --app "$dyld_environment_app" --target 12.0 --arch arm64

alternate_app=$(make_app alternate-command 12.0)
...
legacy_variants_app=$(make_app legacy-load-variants 12.0)
...
wrong_arch_app=$(make_app wrong-arch 12.0)
compile_executable "$wrong_arch_app/Contents/MacOS/barrier" x86_64-apple-macos12 \
    'int main(void) { return 0; }'
expect_failure wrong-arch 'unexpected architectures (x86_64): MacOS/barrier' \
    "$verifier" --app "$wrong_arch_app" --target 12.0 --arch arm64

high_target_app=$(make_app high-target 12.0)
compile_executable "$high_target_app/Contents/MacOS/barrier" arm64-apple-macos13 \
    'int main(void) { return 0; }'
expect_failure high-target 'deployment target 13.0 exceeds 12.0: MacOS/barrier' \
    "$verifier" --app "$high_target_app" --target 12.0 --arch arm64

plugin_id_app=$(make_app plugin-id 12.0)
compile_executable "$plugin_id_app/Contents/MacOS/barrier" arm64-apple-macos12 \
    'int main(void) { return 0; }'
/bin/mkdir -p "$plugin_id_app/Contents/PlugIns/bearer"
printf '%s\n' 'int plugin_fixture(void) { return 0; }' | /usr/bin/xcrun clang -x c - \
    -target arm64-apple-macos12 -dynamiclib \
    -Wl,-install_name,libValidPlugin.dylib \
    -o "$plugin_id_app/Contents/PlugIns/bearer/libValidPlugin.dylib"
"$verifier" --app "$plugin_id_app" --target 12.0 --arch arm64 \
    > "$test_root/plugin-id.log"

unsafe_prefix=/opt/example
unsafe_install_id="$unsafe_prefix/lib/libUnsafeIdentity.dylib"
unsafe_id_app=$(make_app unsafe-id 12.0)
compile_executable "$unsafe_id_app/Contents/MacOS/barrier" arm64-apple-macos12 \
    'int main(void) { return 0; }'
...
expect_failure unsafe-id 'unsafe install identity: MacOS/libUnsafeIdentity.dylib' \
    "$verifier" --app "$unsafe_id_app" --target 12.0 --arch arm64
...
escaping_rpath_app=$(make_app escaping-rpath 12.0)
printf '%s\n' 'int main(void) { return 0; }' \
    | /usr/bin/xcrun clang -x c - -target arm64-apple-macos12 \
...
expect_failure escaping-rpath 'unsafe runpath: MacOS/barrier' \
    "$verifier" --app "$escaping_rpath_app" --target 12.0 --arch arm64
...
unrelated_rpath_app=$(make_app unrelated-rpath 12.0)
...
"$verifier" --app "$unrelated_rpath_app" --target 12.0 --arch arm64 \
...
no_dynamic_context_app=$(make_app no-dynamic-context 12.0)
...
expect_failure no-dynamic-context \
    ...
    "$verifier" --app "$no_dynamic_context_app" --target 12.0 --arch arm64

dynamic_external_app=$(make_app dynamic-external 12.0)
...
expect_failure dynamic-external \
    ...
    "$verifier" --app "$dynamic_external_app" --target 12.0 --arch arm64
...
bare_external_app=$(make_app bare-external 12.0)
...
expect_failure bare-external 'forbidden external dependency: MacOS/barrier' \
    "$verifier" --app "$bare_external_app" --target 12.0 --arch arm64
...
missing_link_app=$(make_app missing-link 12.0)
...
expect_failure missing-link 'unresolved bundled dependency: MacOS/barrier' \
    "$verifier" --app "$missing_link_app" --target 12.0 --arch arm64
...
privacy_app=$(make_app privacy 12.0)
compile_executable "$privacy_app/Contents/MacOS/barrier" arm64-apple-macos12 \
...
expect_failure privacy-string 'local-environment or private-network string found: MacOS/barrier' \
    "$verifier" --app "$privacy_app" --target 12.0 --arch arm64
...
unix_home_app=$(make_app unix-home 12.0)
compile_executable "$unix_home_app/Contents/MacOS/barrier" arm64-apple-macos12 \
...
expect_failure unix-home 'local-environment or private-network string found: MacOS/barrier' \
    "$verifier" --app "$unix_home_app" --target 12.0 --arch arm64
...
windows_home_app=$(make_app windows-home 12.0)
compile_executable "$windows_home_app/Contents/MacOS/barrier" arm64-apple-macos12 \
...
expect_failure windows-home 'local-environment or private-network string found: MacOS/barrier' \
    "$verifier" --app "$windows_home_app" --target 12.0 --arch arm64
...
private_network_app=$(make_app private-network 12.0)
compile_executable "$private_network_app/Contents/MacOS/barrier" arm64-apple-macos12 \
...
expect_failure private-network 'local-environment or private-network string found: MacOS/barrier' \
    "$verifier" --app "$private_network_app" --target 12.0 --arch arm64
...
protected_resource_app=$(make_app protected-resource-path 12.0)
...
expect_failure protected-resource-path \
    ...
    "$verifier" --app "$protected_resource_app" --target 12.0 --arch arm64
...
protected_directory_app=$(make_app protected-directory-path 12.0)
...
expect_failure protected-directory-path \
    'protected or unsafe app pathname found' \
    "$verifier" --app "$protected_directory_app" --target 12.0 --arch arm64
...
unsafe_resource_app=$(make_app unsafe-resource-path 12.0)
...
expect_failure unsafe-resource-path \
    'protected or unsafe app pathname found' \
    "$verifier" --app "$unsafe_resource_app" --target 12.0 --arch arm64
...
private_link_target_app=$(make_app private-link-target 12.0)
...
expect_failure private-link-target \
    ...
    "$verifier" --app "$private_link_target_app" --target 12.0 --arch arm64
...
unsafe_link_target_app=$(make_app unsafe-link-target 12.0)
...
expect_failure unsafe-link-target \
    'protected or unsafe symlink target found' \
    "$verifier" --app "$unsafe_link_target_app" --target 12.0 --arch arm64
...
for wide_encoding in UTF-16LE UTF-16BE UTF-32LE UTF-32BE; do
    ...
    wide_app=$(make_app "privacy-$wide_fixture_name" 12.0)
    ...
    expect_failure "$wide_fixture_name" \
        ...
        "$verifier" --app "$wide_app" --target 12.0 --arch arm64
...
absolute_link_app=$(make_app absolute-link 12.0)
compile_executable "$absolute_link_app/Contents/MacOS/barrier" arm64-apple-macos12 \
    'int main(void) { return 0; }'
...
expect_failure absolute-link 'absolute symlink target found: Resources/private-link' \
    "$verifier" --app "$absolute_link_app" --target 12.0 --arch arm64
...
relative_link_app=$(make_app relative-link 12.0)
compile_executable "$relative_link_app/Contents/MacOS/barrier" arm64-apple-macos12 \
    'int main(void) { return 0; }'
...
expect_failure enumeration-failure 'unable to enumerate verification pathnames' \
    "$verifier" --app "$enumeration_failure_app" --target 12.0 --arch arm64
/bin/chmod 700 \
    "$enumeration_failure_app/Contents/Resources/unreadable-subtree"
if /usr/bin/grep -F "$test_root" \
...
unreadable_file_app=$(make_app unreadable-file 12.0)
...
expect_failure unreadable-file \
    ...
    "$verifier" --app "$unreadable_file_app" --target 12.0 --arch arm64
...
compile_executable "$external_archive_root/bin/probe" arm64-apple-macos12 \
    'int main(void) { return 0; }'
printf '%s\n' 'int external_archive_fixture(void) { return 0; }' \
    | /usr/bin/xcrun clang -x c - -target arm64-apple-macos12 -c \
...
expect_failure external-archive \
    ...
    "$verifier" --root "$external_archive_root" --target 12.0 --arch arm64 \
...
compile_executable "$valid_archive_root/bin/probe" arm64-apple-macos12 \
    'int main(void) { return 0; }'
...
"$verifier" --root "$valid_archive_root" --target 12.0 --arch arm64 \
...
/usr/bin/grep -F $'file\tbin/probe\tarch=arm64\tminos=12.0' \
    "$valid_archive_manifest" >/dev/null
/usr/bin/grep -F $'archive\tlib/libValidArchive.a\tarch=arm64\tminos=12.0' \
    "$valid_archive_manifest" >/dev/null
valid_archive_root_alias="$test_root/valid-archive-root-alias"
/bin/ln -s valid-archive-root "$valid_archive_root_alias"
"$verifier" --root "$valid_archive_root_alias" --target 12.0 --arch arm64 \
